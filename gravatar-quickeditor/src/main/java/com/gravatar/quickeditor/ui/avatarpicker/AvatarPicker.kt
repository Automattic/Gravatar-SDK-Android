...
private fun AvatarPickerAction.handle(
    ...
    when (this) {
        is AvatarPickerAction.AvatarSelected -> {
            ...
            scope.launch {
                snackState.showQESnackbar(
                    message = context.getString(R.string.gravatar_avatar_selected_confirmation),
        ...
        AvatarPickerAction.AvatarSelectionFailed -> {
            scope.launch {
                snackState.showQESnackbar(
                    message = context.getString(R.string.gravatar_avatar_selection_error),
                    withDismissAction = true,
...
private val SectionError.titleRes: Int
    @StringRes get() = when (this) {
        is SectionError.InvalidToken -> R.string.gravatar_avatar_picker_session_error_title
        SectionError.NoInternetConnection -> R.string.gravatar_avatar_picker_network_error_title
        SectionError.ServerError,
        SectionError.Unknown,
        -> R.string.gravatar_avatar_picker_server_error_title
    }

private val SectionError.messageRes: Int
    @StringRes get() = when (this) {
        is SectionError.InvalidToken -> if (showLogin) {
            R.string.gravatar_avatar_picker_session_error_message
        } else {
            R.string.gravatar_avatar_picker_session_error_no_login_message
        }

        SectionError.NoInternetConnection -> R.string.gravatar_avatar_picker_network_error_message
        SectionError.ServerError -> R.string.gravatar_avatar_picker_server_error_message
        SectionError.Unknown -> R.string.gravatar_avatar_picker_unknown_error_message
    }

private val SectionError.buttonTextRes: Int
    @StringRes get() = when (this) {
        is SectionError.InvalidToken -> if (showLogin) {
            R.string.gravatar_avatar_picker_session_error_cta
        } else {
            R.string.gravatar_avatar_picker_session_error_close_cta
        }

        SectionError.NoInternetConnection,
        SectionError.ServerError,
        SectionError.Unknown,
        -> R.string.gravatar_avatar_picker_error_retry_cta
    }
